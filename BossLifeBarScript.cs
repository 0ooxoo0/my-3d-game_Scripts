...
namespace Using
{
    public class BossLifeBarScript : MonoBehaviour
    {
        ...
        public float life = 0; // total de vida
        private float filler = 30; // valor pelo qual a vida sera multiplicada
        public float ghost = 0; // ghost da barra de vida
        ...
        {
            int prosholIgry = 0;
            if(File.Exists(Application.persistentDataPath + "/ProsholIgry" + ("" + 0 + PlayerPrefs.GetInt("SaveBlock")) + Application.version + ".txt"))
            prosholIgry = int.Parse(File.ReadAllText(Application.persistentDataPath + "/ProsholIgry" + ("" + 0 + PlayerPrefs.GetInt("SaveBlock")) + Application.version + ".txt"));
            maxLife = maxLife * (1 + prosholIgry);
        ...
        {
            //if (life <= 0) // mata o boss caso ainda nao tenha feito
            //{
            //    Die();
            //}
